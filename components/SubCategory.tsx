  ...
  return (
    <div>
      <style>{`
        ...
        .btn-primary { background: #F8F5F0; color: #4A2C1D; border: none; border-radius: 8px; padding: 10px 20px; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; cursor: pointer; transition: background 0.15s; }
        .btn-primary:hover { background: #E67E22; }
        .btn-ghost { background: transparent; color: #A89D8E; border: 1px solid #7A4C36; border-radius: 8px; padding: 8px 14px; font-family: 'DM Sans', sans-serif; font-size: 12px; cursor: pointer; transition: all 0.15s; }
        .btn-ghost:hover { color: #E67E22; border-color: #E67E2240; }
        .input { background: #362015; border: 1px solid #7A4C36; border-radius: 8px; color: #F8F5F0; font-family: 'DM Sans', sans-serif; font-size: 13px; padding: 10px 14px; width: 100%; outline: none; transition: border-color 0.15s; }
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0, flex: 1 }}>
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize: "clamp(12px, 2vw, 14px)", color: "#F8F5F0", wordBreak: "break-word" }}>{cat.name}</div>
                </div>
              </div>
              <button className="btn-ghost" onClick={() => remove(cat._id!)} style={{ flexShrink: 0 }}>✕</button>
            </div>

            <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #7A4C36", display: "flex", justifyContent: "flex-start" }}>
      ...
      {showModal && (
        <div className="overlay" onClick={() => setShowModal(false)}>
          <div className="card" style={{ width: "clamp(300px, 90vw, 400px)", padding: "clamp(16px, 4vw, 28px)" }} onClick={(e) => e.stopPropagation()}>
            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(16px, 4vw, 18px)", fontWeight: 700, color: "#F8F5F0", marginBottom: 20 }}>
...
function FieldLabel({ label, children }: FieldProps) {
  return (
    <div>
      <div style={{ fontSize: 11, fontWeight: 600, color: "#A89D8E", marginBottom: 6 }}>
        {label}
      </div>
      {children}
    </div>
...
function PageHeader({ title, sub, onAdd }: HeaderProps) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, width: "100%" }}>
      <div>
        <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(20px, 5vw, 26px)", fontWeight: 800, color: "#F8F5F0", margin: 0 }}>{title}</h1>
        <p style={{ fontSize: 13, color: "#938575", margin: "4px 0 0 0" }}>{sub}</p>
      </div>
