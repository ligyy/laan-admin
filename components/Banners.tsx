  ...
  return (
    <div>
      <style>{`
        ...
        .btn-primary { background: #F8F5F0; color: #4A2C1D; border: none; border-radius: 8px; padding: 10px 20px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s ease; }
        .btn-primary:hover { background: #E67E22; }
        .btn-primary:disabled { opacity: 0.5; cursor: not-allowed; }
        .btn-ghost { background: transparent; color: #A89D8E; border: 1px solid #7A4C36; border-radius: 8px; padding: 6px 12px; font-size: 12px; cursor: pointer; transition: all 0.2s ease; }
        ...
        .toggle-slider { position: absolute; inset: 0; background: #7A4C36; border-radius: 22px; cursor: pointer; transition: 0.2s; }
        .toggle-slider:before { content: ''; position: absolute; height: 16px; width: 16px; left: 3px; bottom: 3px; background: #A89D8E; border-radius: 50%; transition: 0.2s; }
        input:checked + .toggle-slider { background: #D1C9BE; }
        input:checked + .toggle-slider:before { transform: translateX(16px); background: #F8F5F0; }
      
        // .upload-box:hover { border-color: #D1C9BE; }
        ...
        .banner-preview-label { font-size: 10px; font-weight: 600; color: #938575; text-transform: uppercase; letter-spacing: 0.05em; }
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12, width: "100%" }}>
        <div>
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(20px, 5vw, 26px)", fontWeight: 800, color: "#F8F5F0", margin: 0 }}>Banners</h1>
          <p style={{ fontSize: 13, color: "#938575", margin: "4px 0 0 0" }}>{banners.filter(b => b.isActive).length} active</p>
        </div>
      ...
      {showModal && (
        <div className="overlay" onClick={() => setShowModal(false)}>
          <div className="card" style={{ width: "clamp(300px, 90vw, 500px)", padding: "clamp(16px, 5vw, 32px)", maxHeight: "90vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(16px, 4vw, 20px)", fontWeight: 700, color: "#F8F5F0", marginBottom: 20 }}>
              New Banner
            </div>
                ...
                <label className="upload-box" style={{ cursor: "pointer", padding: "20px 16px" }}>
                  {desktopImage ? (
                    <div style={{ position: "relative", width: "100%", height: "150px" }}>
                      ...
                      />
                      <div style={{ position: "absolute", top: 8, right: 8, background: "#D1C9BE", color: "#F8F5F0", padding: "4px 8px", borderRadius: 4, fontSize: 11, fontWeight: 600 }}>
                        Selected
                      </div>
                    </div>
                  ) : (
                    <div style={{ padding: "30px 0" }}>
                      <div style={{ fontSize: 13, color: "#A89D8E", fontWeight: 500 }}>📁 Click to upload desktop image</div>
                      <div style={{ fontSize: 11, color: "#938575", marginTop: 6 }}>Recommended: 1920x400px</div>
                    </div>
                  )}
              ...
              <Field label="Mobile Image (Required)">
                <label className="upload-box" style={{ cursor: "pointer", padding: "20px 16px" }}>
                  {mobileImage ? (
                    ...
                    <div style={{ padding: "30px 0" }}>
                      <div style={{ fontSize: 13, color: "#A89D8E", fontWeight: 500 }}>📱 Click to upload mobile image</div>
                      <div style={{ fontSize: 11, color: "#938575", marginTop: 6 }}>Recommended: 540x600px</div>
                    </div>
                  )}
