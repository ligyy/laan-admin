  ...
  return (
    <div>
      <style>{`
        ...
        .btn-primary:hover:not(:disabled) { background: #BCAE9F; }
        .btn-primary:disabled { opacity: 0.6; cursor: not-allowed; }
        .btn-ghost { background: transparent; color: #A89D8E; border: 1px solid #7A4C36; border-radius: 8px; padding: 8px 14px; font-size: 12px; cursor: pointer; transition: all 0.15s; }
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 16, marginTop: 24 }}>
        {subcategories.map(sub => (
          <div key={sub._id} className="card" style={{ padding: 20 }}>
            <div style={{ fontWeight: 600, fontSize: 14, color: "#F8F5F0", marginBottom: 16 }}>{sub.name}</div>
            <div style={{ display: "flex", gap: 8, borderTop: "1px solid #7A4C36", paddingTop: 14 }}>
        ...
        {subcategories.length === 0 && (
          <div style={{ color: "#938575", fontSize: 14, padding: 20 }}>No Brands yet.</div>
        )}
      </div>
